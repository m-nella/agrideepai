...
// --- System Prompt (Creator Identity) ---
const SYSTEM_PROMPT = `
...
app.post('/api/chat/conversations/:id/regenerate', authenticate, async (req, res) => {
  // Same as before but with model fallback
  try {
    const conversationId = req.params.id;
...
// --- Share endpoint: generate a public link for a chat ---
app.post('/api/chat/share/:id', authenticate, async (req, res) => {
  try {
    ...
    // Generate a unique share token (use nanoid or similar; we'll use a random string)
    const shareToken = require('crypto').randomBytes(16).toString('hex');
    // Store in a 'shared_chats' table (we'll assume we have one)
    // For simplicity, we'll store in a JSON field in the conversation or a separate table.
    // We'll create a table `shared_links` with columns: id, conversation_id, token, created_at.
    // We'll implement a minimal version here.
    const { data, error } = await supabase
      .from('shared_links')
      .insert({ conversation_id: id, token: shareToken })
      .select()
      .single();
    if (error) throw error;
...
app.listen(PORT, () => {
  console.log(`✅ AgriDeepAI server running on port ${PORT}`);
});
