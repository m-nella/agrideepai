...
function getModel() {
  ...
  for (const name of MODELS) {
    try {
      ...
      console.log(`✅ Using Gemini: ${name}`);
      activeModel = model;
      return model;
    } catch (e) { /* try next */ }
  }
  activeModel = genAI.getGenerativeModel({ model: 'gemini-pro' });
  return activeModel;
}
...
app.post('/api/auth/resend-verification', async (req, res) => {
  try {
    ...
    const code = generateCode();
    verificationStore[user.id] = { code, expires: Date.now() + 10 * 60 * 1000 };
    await resend.emails.send({ from: process.env.RESEND_FROM_EMAIL, to: email, subject: 'Verify', html: `<p>Code: ${code}</p>` });
...
app.post('/api/auth/change-password', authenticate, async (req, res) => {
  try {
    ...
    const { error: signInError } = await supabase.auth.signInWithPassword({ email: req.user.email, password: currentPassword });
...
app.post('/api/auth/change-email', authenticate, async (req, res) => {
  try {
    ...
    res.json({ message: 'Email change requested. Verify new email.' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/chat/conversations/:id/messages', authenticate, upload.single('file'), async (req, res) => {
  try {
    ...
    // Verify ownership
    const { data: conv } = await supabase.from('conversations').select('id').eq('id', conversationId).eq('user_id', req.user.id).single();
    ...
    if (file) {
      ...
      const { data: uploadData } = await supabase.storage.from('uploads').upload(filePath, file.buffer, { contentType: file.mimetype });
      const { publicURL } = supabase.storage.from('uploads').getPublicUrl(filePath);
      fileMetadata = { filename: file.originalname, storage_path: filePath, mime_type: file.mimetype, size: file.size, public_url: publicURL };
    ...
    // Search
    let searchResults = null;
    if (TAVILY_API_KEY) searchResults = await tavilySearch(message || 'agriculture update');
    ...
    if (file && file.mimetype.startsWith('image/')) {
      ...
      result = await chat.sendMessageStream([{ text: finalPrompt }, { inlineData: { data: base64, mimeType: file.mimetype } }]);
    } else {
    ...
    await supabase.from('messages').insert({ conversation_id: conversationId, role: 'assistant', content: fullResponse, files: sourcesData ? [{ sources: sourcesData }] : null });
...
app.listen(PORT, () => console.log(`✅ AgriDeepAI server on port ${PORT}`));
